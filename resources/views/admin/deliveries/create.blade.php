...
<div class="card mt-6 md:w-10/12 bg-white shadow-lg p-3 rounded-sm mx-auto flex items-center justify-between">
    ...
    <div>
        <a href="{{route('deliveries.pendings', $job->subject->id)}}"
...
<div class="flex justify-center mt-2 mb-8">
    <div class="card bg-white rounded-sm w-full md:w-10/12 p-4 shadow-lg">
        <div class=" w-full flex relative items-center border-b mb-2 pb-3">
            ...
            <div class="w-3/12">
                ...
                <span
                    class="bg-green-200 text-green-800 float-right rounded-full px-2 py-1 text-xs font-medium hidden md:block">{{$delivery->state($delivery->state)}}</span>
                @endif
                @if ($delivery->state($delivery->state) === "En corrección")
                <span
                    class="bg-gray-200 text-gray-800 float-right rounded-full px-2 py-1 text-xs font-medium hidden md:block">{{$delivery->state($delivery->state)}}</span>
                @endif
                ...
                <span
                    class="bg-red-200 text-red-800 float-right rounded-full px-2 py-1 text-xs font-medium hidden md:block">{{$delivery->state($delivery->state)}}</span>
                @endif

                @else
                <p
                    class="bg-purple-200 text-purple-800 float-right rounded-full px-2 py-1 text-xs font-medium hidden md:block">
                    Sin entrega</p>
                @endif
            </div>
        ...
        <div class="text-right">
            ...
            @if ($delivery->state($delivery->state) === "Aprobado")
            <span
                class="rounded-full text-green-800 bg-green-200 px-2 py-1 text-xs font-medium md:hidden">{{$delivery->state($delivery->state)}}</span>
            @endif
            @if ($delivery->state($delivery->state) === "En corrección")
            <span
                class="rounded-full text-gray-800 bg-gray-200 px-2 py-1 text-xs font-medium md:hidden">{{$delivery->state($delivery->state)}}</span>
            @endif
            {{-- @if ($delivery->state($delivery->state) === "Por Corregir")
            ...
            @if ($delivery->state($delivery->state) === "Rehacer")
            <span
                class="rounded-full text-red-800 bg-red-200 px-2 py-1 text-xs font-medium md:hidden">{{$delivery->state($delivery->state)}}</span>
            @endif

            @else
            <span class="rounded-full text-purple-800 bg-purple-200 px-2 py-1 text-xs font-medium md:hidden">Sin
                entrega</span>
            @endif
        </div>
        ...
        <div class="alert flex flex-row items-center bg-gray-100 p-5 rounded border-b-2 border-blue-300 mt-6 mb-6">
            <div
                class="alert-icon flex items-center bg-gray-100 border-2 border-blue-300 justify-center h-10 w-10 flex-shrink-0 rounded-full">
                <span class="text-blue-500">
                    <svg aria-hidden="true" data-prefix="far" data-icon="laugh-beam"
                        class="svg-inline--fa fa-laugh-beam fa-w-16 w-8 h-8 text-blue-600"
            ...
            <div class="alert-content ml-4 w-full">
                ...
                <div class="alert-description text-sm text-blue-600 flex py-1 items-center">
                    <a id="verFileEntrega" target="_blank" href="" class="flex hover:text-blue-800 items-center">
                        Ver mi entrega
                        <svg aria-hidden="true" data-prefix="fas" data-icon="file-import"
                            class="ml-2 svg-inline--fa fa-file-import fa-w-16 fa-w-16 w-4 h-4"
                ...
                <p class="text-sm text-blue-600 flex py-1 items-center">Sin archivo
                    <svg aria-hidden="true" data-prefix="fas" data-icon="exclamation-circle"
                        class="svg-inline--fa fa-exclamation-circle fa-w-16 w-4 h-4 ml-2"
                        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                        <path fill="currentColor"
        ...
        <div class=" w-full flex relative items-center border-b mb-2 py-3">
            ...
            <div class="flex w-full pt-1">
                ...
                <a id="descargarFile" href="{{route('descargarJob', $job)}}"
                    class="btn btn-default mr-3 py-1 px-2 rounded-md hidden md:flex">
        ...
        {{-- @if ($delivery)
            @if ($delivery->file_path)
                <div class="flex justify-center p-2 mt-2">
                    <a id="descargarFile" target="_blank" href="{{asset($delivery->file_path)}}" class="btn
        btn-secondary flex" >Ver Mi Entrega
        <svg aria-hidden="true" data-prefix="fas" data-icon="file-import"
            class="svg-inline--fa fa-file-import fa-w-16 ml-2 fa-w-16 w-5 h-5" xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512 512">
            <path fill="currentColor"
                d="M16 288c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h112v-64zm489-183L407.1 7c-4.5-4.5-10.6-7-17-7H384v128h128v-6.1c0-6.3-2.5-12.4-7-16.9zm-153 31V0H152c-13.3 0-24 10.7-24 24v264h128v-65.2c0-14.3 17.3-21.4 27.4-11.3L379 308c6.6 6.7 6.6 17.4 0 24l-95.7 96.4c-10.1 10.1-27.4 3-27.4-11.3V352H128v136c0 13.3 10.7 24 24 24h336c13.3 0 24-10.7 24-24V160H376c-13.2 0-24-10.8-24-24z" />
        </svg>
    ...
    <div class="flex flex-wrap my-5 border">
        <div class="w-full md:w-full  py-4 text-gray-700 text-sm px-3 mb-6 md:mb-0">
            <div class="border-b">
                <label class="block uppercase tracking-wide text-gray-700 text-md font-bold mb-2" for="grid-state">
                    Mi Entrega
                </label>
            ...
            <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mt-5 mb-2" for="grid-last-name">
                ...
                <form method="POST" action="{{route('deliver.store')}}" class="mx-auto" id="delivery"
                    ...
                    <div class="flex flex-wrap my-5">
                        <div class="w-full md:w-full md:mb-0 mb-6">
                            ...
                            @if($delivery)
                            @if($delivery->link)
                            <input type="text" name="link" id="link" value="{{$delivery->link}}"
                                class="form-input w-full block" id="grid-last-name" type="text"
                                placeholder="Link del video" onchange="setLink()">
                            @else
...
const boxFile = document.getElementById("boxFile");
...
if (archivos != null) {
    archivos.addEventListener("change", () => {
            ...
            reader.onload = function(e) {
                ...
                div.innerHTML = `
                    <div id='${name}' class='relative p-2'>
                        <img class='w-full h-32 bg-cover bg-center rounded-md' src='${e.target.result}' >
                    </div>
                `;
        ...
        selecteds.innerHTML =
            "<button id='boton' type='button' onclick='limpiarGaleria()' class='border border-gray-400 rounded-sm px-3 py-1 hover:bg-gray-400 focus:shadow-outline focus:outline-none'>Cancelar</button>";
        setTimeout(() => {
            cargando.classList.toggle("hidden");
        }, 3000);
    });
...
function limpiarGaleria() {
    selecteds.innerHTML = "Adjuntar Imágenes";
...
<script>
    ...
    function limpiarFile(){
        ...
        selectedVideo.innerHTML = 'Adjuntar Archivo';
    }
</script>
...
<script>
    function setName(){
        ...
        selected.innerHTML = cad[2] + ' ' +"<button id='boton' type='button' onclick='limpiarFile()' class='border border-gray-400 rounded-sm px-3 py-1 hover:bg-gray-400 focus:shadow-outline focus:outline-none'>x</button>";
