...
<div class="container font-montserrat text-sm ">
    <div class="card  rounded-sm bg-gray-100 mx-auto md:mt-10 shadow-lg">
        <div class="card-title bg-white w-full p-1 md:p-5  border-b flex items-center justify-between md:justify-between ">
           <h1 class="text-teal-600 font-semibold">Subjects</h1>
            <a href="{{route('subjects.create')}}" class="bg-teal-600 text-white text-sm p-2 shadow-lg hover:text-gray-700">New Subject</a>
        ...
        <div class="card-body py-2">
            ...
            <table class="text-gray-700">
                <thead>
                    <tr class="text-center">
                        ...
                        <th class="md:w-5/12 w-2/12 px-10 ">Materia</th>
                        <th class="w-3/12 ">Cod</th>
                        {{-- Agregue ultima actualizacion s --}}
                        <th class="w-2/12 ">Curso</th>

                        <th class="w-3/12 px-10">Acciones</th>
                ...
                <tbody>
                    @foreach ($subjects as $subject)
                        <tr class="text-center">
                        <td  class="w-1/12 px-10 hidden md:block ">{{$subject->id}}</td>
                        <td  class="w-3/12 ">{{$subject->name}}</td>
                        <td  class="w-3/12">{{$subject->code}}</td>
                        <td  class="w-2/12">{{$subject->course->code}}</td>
                        <td  class="md:w-3/12 w-9/12">
                            <div class="flex justify-center">
                                <a href="{{route('subjects.edit', $subject->id)}}" class="mx-1 text-teal-400">
            ...
            @else
                <h2>No posee materias registradas</h2>
            @endif
