...
<div class="container font-montserrat text-sm ">
    <div class="card  rounded-sm bg-gray-100 mx-auto md:mt-10 shadow-lg">
        <div class=" card-title bg-white w-full p-1 md:p-5  border-b flex items-center justify-between md:justify-between ">
           <h1 class="text-teal-600 font-semibold">Alumnos</h1>
           <div>

            <select name="curso" id="" required>
                <option value="" >Seleccione curso de destino</option>
                @foreach ($cursos as $curso)
                    <option value="{{$curso->id}}">{{$curso->name}} - {{$curso->cicle}}</option>
                @endforeach
            </select>
           </div>
            ...
            <table class="text-gray-700 table w-full">
                <thead>
                    <tr class="flex justify-between text-center text-xs md:text-lg">
                        ...
                        <th><input type="checkbox" id="checkAll" onchange="setAll()"></th>
                        <th class="font-normal w-2/12 hidden md:block">Name</th>
                        <th class="font-normal w-3/12 md:w-3/12 ">Email</th>
                        <th class="font-normal w-3/12 md:w-2/12 ">Dni</th>
                ...
                <tbody class="bg-white ">
                    ...
                    <tr class="text-xs md:text-lg flex justify-between text-center items-center border-t border-gray-600 mx-2" id="row">
                        <td class="w-1/12 hidden md:block" >
                            @if($student->photo)
                                <img src="{{asset($student->photo)}}" class="rounded-full h-12 w-12" alt="">
                            @else
                        ...
                        <td>
                            ...
                            <input class="studentRow" type="checkbox" name="matriculados[{{$student->id}}]" >

                        </td>
                        <td class="w-1/12 text-xs hidden md:block " > {{$student->name}}</td>
                        <td class="w-1/12 text-xs hidden md:block" > {{$student->email}}</td>
                        <td class="w-2/12 text-xs hidden md:block text-center" > {{ $student->dni}}</td>
                        <td class="w-3/12 text-xs md:w-3/12 " >
                            <div class="flex justify-center px-2">
                            <a href="{{route('user.edit',$student->user_id)}}" class="mx-1 text-teal-400">
    ...
    <button class="w-6/12 bg-teal-600 text-white">Mover alumnos</button>
</form>
@endsection
@push('js')
    ...
    <script>
        function setAll(){
           let ctrl = document.getElementById('checkAll')
            if (ctrl.checked) {
                checkEvery()
            }else{
                uncheckEvery()
        ...
        function checkEvery(){
            checkboxes = document.getElementsByClassName('studentRow')
