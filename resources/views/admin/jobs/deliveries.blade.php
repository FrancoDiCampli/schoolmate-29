...
<div class="flex justify-center mt-2 mb-8">
    <div class="card bg-white rounded-sm w-full md:w-10/12 p-4 shadow-lg">
        ...
        <div class="flex w-auto">
            <a target="_blank" href="{{route('entregasPDF', $job->id)}}"
                class="btn btn-default py-1 px-3 rounded-sm md:hidden">
                <svg aria-hidden="true" data-prefix="far" data-icon="file-pdf"
                    class="h-5 w-5 svg-inline--fa fa-file-pdf fa-w-12" xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 384 512">
        ...
        <div class="flex w-auto">
            <a target="_blank" href="{{route('entregasPDF', $job->id)}}"
                class="items-center btn btn-default hidden md:flex">Exportar
                <svg aria-hidden="true" data-prefix="far" data-icon="file-pdf"
                    class="ml-2 h-6 w-6 svg-inline--fa fa-file-pdf fa-w-12" xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 384 512">
                    <path fill="currentColor"
                        d="M369.9 97.9L286 14C277 5 264.8-.1 252.1-.1H48C21.5 0 0 21.5 0 48v416c0 26.5 21.5 48 48 48h288c26.5 0 48-21.5 48-48V131.9c0-12.7-5.1-25-14.1-34zM332.1 128H256V51.9l76.1 76.1zM48 464V48h160v104c0 13.3 10.7 24 24 24h104v288H48zm250.2-143.7c-12.2-12-47-8.7-64.4-6.5-17.2-10.5-28.7-25-36.8-46.3 3.9-16.1 10.1-40.6 5.4-56-4.2-26.2-37.8-23.6-42.6-5.9-4.4 16.1-.4 38.5 7 67.1-10 23.9-24.9 56-35.4 74.4-20 10.3-47 26.2-51 46.2-3.3 15.8 26 55.2 76.1-31.2 22.4-7.4 46.8-16.5 68.4-20.1 18.9 10.2 41 17 55.8 17 25.5 0 28-28.2 17.5-38.7zm-198.1 77.8c5.1-13.7 24.5-29.5 30.4-35-19 30.3-30.4 35.7-30.4 35zm81.6-190.6c7.4 0 6.7 32.1 1.8 40.8-4.4-13.9-4.3-40.8-1.8-40.8zm-24.4 136.6c9.7-16.9 18-37 24.7-54.7 8.3 15.1 18.9 27.2 30.1 35.5-20.8 4.3-38.9 13.1-54.8 19.2zm131.6-5s-5 6-37.3-7.8c35.1-2.6 40.9 5.4 37.3 7.8z" />
                </svg>
                ...
                <table class="table-auto border-collapse w-full">
                    ...
                    <tbody class="text-sm font-normal text-gray-700">
                        ...
                        <tr class="hover:bg-gray-100 border-b border-gray-200 bg-white text-sm">
                            ...
                            <td class="px-4 py-2 mt-1">{{$entrega->updated_at->format('d-m-Y H:i')}}</td>
                            <td class="px-4 py-2">
                                @if ($entrega->state($entrega->state) === "Aprobado")
                                <span
                                    class="bg-green-200 py-1 px-2 rounded-full text-green-800">{{$entrega->state($entrega->state)}}</span>
                                @endif
                                @if ($entrega->state($entrega->state) === "En corrección")
                                <span
                                    class="bg-gray-200 py-1 px-2 rounded-full text-gray-800">{{$entrega->state($entrega->state)}}</span>
                                @endif
                                ...
                                @if ($entrega->state($entrega->state) === "Rehacer")
                                <span
                                    class="bg-red-200 py-1 px-2 rounded-full text-red-800">{{$entrega->state($entrega->state)}}</span>
                                @endif

                            </td>
                            <td class="px-4 py-2">
                                <div class="flex">
                                    <a href="{{route('job.delivery', $entrega->id)}}"
                                        class="mx-1 text-blue-400 hover:bg-gray-200 rounded-full p-2 focus:bg-gray-300">
                                        <svg aria-hidden="true" data-prefix="fas" data-icon="info"
                                            class="h-4 w-4 svg-inline--fa fa-info fa-w-6"
