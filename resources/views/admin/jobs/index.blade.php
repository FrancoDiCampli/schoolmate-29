    ...
    <div class="w-auto mx-auto flex items-center justify-between p-5">
        <form action="{{route('searchJobs')}}" method="POST">
            ...
            <div class="border border-gray-400 bg-white h-10 rounded-sm py-1 content-center flex items-center">
                <input type="text" hidden name="subjectID" id="" value="{{$subject->id}}">
                <input name="search" type="text"
                    class="bg-transparent focus:outline-none w-full text-sm p-2 text-gray-800" placeholder="Buscar...">
                <button type="submit"
        ...
        @role('teacher')
        <a href="{{route('jobs.create', $subject->id)}}" class="hidden md:block btn btn-primary md:m-0 m-3">Nueva
            Tarea</a>
        ...
        <div class=" w-full flex relative items-center ">
            ...
            <div class="w-auto md:w-6/12">
                <a href="{{route('deliver', $job->id)}}">
                    <p class="md:text-md text-sm font-semibold text-gray-900 -mt-1 md:pt-0 pt-2 hover:text-primary-400">
                        {{$job->title}} </p>
                </a>
            ...
            <div class="w-auto md:w-6/12">
                <a href="{{route('jobs.showJob', $job->id)}}">
                    <p
                        class="md:text-md text-sm font-semibold text-gray-900 -mt-1 md:pt-0 pt-2 hover:text-primary-400 break-all">
                        {{$job->title}} </p>
                </a>
                <p class="text-gray-700 font-light text-xs">Fecha limite: {{$job->end->format('d-m-Y')}} </p>
            </div>
            ...
            @role('teacher')
            <div class="md:w-6/12 text-center mr-3">
            ...
            <div class="md:w-6/12 text-right mr-3 text-xs md:text-base hidden md:block">
                ...
                @case("En corrección")
                <span
                    class="bg-gray-200 py-1 px-2 rounded-full text-gray-800">{{$delivery->state($delivery->state)}}</span>
                @break

                @case("Aprobado")
                <span
                    class="bg-green-200 py-1 px-2 rounded-full text-green-800">{{$delivery->state($delivery->state)}}</span>
                @break
                ...
                @case("Rehacer")
                <span
                    class="bg-red-200 py-1 px-2 rounded-full text-red-800">{{$delivery->state($delivery->state)}}</span>
                @break
